...
const recentTransactionsColumns: ColumnDef<Transaction>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      let text = 'N/A';
      if ('description' in transaction && transaction.description) text = transaction.description;
      else if ('full_name' in transaction && transaction.full_name) text = `Pago de ${transaction.full_name}`;
...
function Dashboard() {
  ...
  const isAdminOrFinanzas = useMemo(() => {
    ...
    // Buscamos de forma insensible a mayúsculas/minúsculas
    return roles.some(role => 
      ['admin', 'finanzas'].includes(role.toLowerCase())
    );
  }, [roles]);

  // Debug log para verificar roles en consola
  useEffect(() => {
    if (!authLoading) {
      console.log("Dashboard - Roles detectados:", roles);
      console.log("Dashboard - ¿Es Admin o Finanzas?:", isAdminOrFinanzas);
    }
  }, [roles, authLoading, isAdminOrFinanzas]);
  ...
  const periodOptions = useMemo(() => {
    const dates = [...ingresos, ...gastos].map(i => parseISO(i.date));
  ...
  const socioStats = useMemo(() => {
    ...
    const pagadosDnis = new Set(ingresos.filter(i => isAfter(parseISO(i.date), currentMonthStart) && i.amount > 0).map(i => i.dni));
  ...
  const chartData = useMemo(() => {
    ...
    [...filteredData.ingresos, ...filteredData.gastos].forEach(item => {
      const key = format(parseISO(item.date), 'MMM yy', { locale: es });
  ...
  const recentTransactions = useMemo(() => {
    return [...ingresos, ...gastos].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5);
  }, [ingresos, gastos]);

  const specialStats = useMemo(() => {
    const annulled = ingresos.filter(i => i.transaction_type === 'Anulacion').length;
    const returned = Math.abs(ingresos.filter(i => i.transaction_type === 'Devolucion').reduce((s, i) => s + i.amount, 0));
    return { annulled, returned };
  }, [ingresos]);

  // Pantalla de carga mientras se verifican los roles
  if (authLoading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#F8F9FC]">
        ...
        <p className="text-gray-500 font-bold animate-pulse">Verificando credenciales...</p>
      </div>
    );
  }
    ...
    <div className="pb-20 bg-[#F8F9FC] min-h-screen">
      ...
      <div className="container mx-auto px-8 -mt-10 relative z-20">
        <Tabs defaultValue="titulares" className="space-y-8">
          {/* SOLO ADMIN/FINANZAS VEN LAS PESTAÑAS */}
          {isAdminOrFinanzas && (
          ...
          <TabsContent value="titulares" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className={cn(
              ...
              {/* TARJETAS DE PAGOS: SOLO PARA ADMIN/FINANZAS */}
              {isAdminOrFinanzas && (
                <>
            ...
            {/* SECCIÓN DE EQUIPO Y DISTRIBUCIÓN: SOLO ADMIN/FINANZAS */}
            {isAdminOrFinanzas && (
          ...
          {/* --- PESTAÑA FINANZAS: SOLO ADMIN/FINANZAS --- */}
          {isAdminOrFinanzas ? (
            <TabsContent value="finanzas" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-white rounded-2xl shadow-sm flex items-center justify-center"><Activity className="text-[#9E7FFF] w-5 h-5" /></div>
                      <h2 className="text-2xl font-black text-gray-900">Actividad Reciente</h2>
                    </div>
            ...
            /* VISTA DE BLOQUEO PARA OTROS ROLES (INGENIERO) SI INTENTAN ACCEDER A FINANZAS */
