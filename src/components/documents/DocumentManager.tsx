...
export default function DocumentManager({ isAdmin }: DocumentManagerProps) {
  ...
  // --- LÓGICA DE BÚSQUEDA MEJORADA (TOKENIZADA) ---
  useEffect(() => {
    const performSearch = async () => {
      ...
      try {
        ...
        // 1. Buscamos de forma amplia en el servidor por el primer token
        const { data, error } = await supabase
          .from('socio_titulares')
          .select(`
            id, nombres, apellidoPaterno, apellidoMaterno, dni, localidad, mz, lote,
            ingresos (receipt_number)
          `)
        ...
        // 2. Filtramos localmente con TODOS los tokens (AND logic)
        const filtered = (data || []).map(s => ({
          ...s,
          // Aplanamos el nro_recibo del último ingreso para la búsqueda
          nro_recibo: s.ingresos?.[0]?.receipt_number || ''
        })).filter(socio => {
            ...
            {exists ? (
              ...
              allowManualUpload ? (
                ...
                <div className={cn(
                  ...
                  {isReceipt ? 'Esperando Pago' : 'Generación Automática'}
                </div>
              )
            )}
          </div>
              ...
              {rawSearchResults.length > 0 && (
                <div className="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl max-h-80 overflow-auto p-2 animate-in fade-in zoom-in-95 duration-200">
                  <div className="px-3 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-50 mb-1">
                    Resultados encontrados ({rawSearchResults.length})
                  </div>
                    ...
                    >
                      ...
                      {socio.nro_recibo && (
                        <Badge className="bg-amber-100 text-amber-700 border-amber-200 font-mono text-[10px] px-2 py-1">
                          <Ticket className="w-3 h-3 mr-1 inline" /> {socio.nro_recibo}
                        </Badge>
                      )}
                    </div>
                  ))}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-100 pb-6 gap-4">
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-3">
                ...
                {selectedSocio.nro_recibo && (
                  <Badge className="bg-gradient-to-r from-amber-500 to-orange-600 text-white border-none shadow-lg px-4 py-2 text-sm font-bold flex items-center gap-2 animate-pulse-subtle">
                    <Ticket className="w-4 h-4" />
                    RECIBO: {selectedSocio.nro_recibo}
                  </Badge>
                )}
              </div>
            </div>
